...
QuadTreeClass::QuadTreeClass( DMD3D* _parent ) : DMSceneObject( _parent )
{
	m_vertexList = 0;
	m_parentNode = 0;
}
...
bool QuadTreeClass::Initialize( TerrainClass* terrain )
{
	int vertexCount;
...
void QuadTreeClass::Render( std::unique_ptr<DMFrustum>& frustum, TerrainShaderClass* shader )
{
	...
	RenderNode( m_parentNode, frustum, shader );
...
void QuadTreeClass::CreateTreeNode( NodeType* node, float positionX, float positionZ, float width )
{
	...
	m_dmd3d->GetDevice()->CreateBuffer( &vertexBufferDesc, &vertexData, &node->vertexBuffer );
	...
	m_dmd3d->GetDevice()->CreateBuffer( &indexBufferDesc, &indexData, &node->indexBuffer );
...
void QuadTreeClass::RenderNode( NodeType* node, std::unique_ptr<DMFrustum>& frustum, TerrainShaderClass* shader )
{
	bool result;
	...
	for( i = 0; i<4; i++ )
	{
		...
		{
			count++;
			RenderNode( node->nodes[i], frustum, shader );
		}
	}
	...
	m_dmd3d->GetDeviceContext()->IASetVertexBuffers( 0, 1, &node->vertexBuffer, &stride, &offset );

	// Set the index buffer to active in the input assembler so it can be rendered.
	m_dmd3d->GetDeviceContext()->IASetIndexBuffer( node->indexBuffer, DXGI_FORMAT_R32_UINT, 0 );
	...
	shader->RenderShader( indexCount );
