...
bool DMSkyPlane::Initialize( ID3D11Device* device, WCHAR* cloudTextureFilename, WCHAR* perturbTextureFilename )
{
...
void DMSkyPlane::Render( ID3D11DeviceContext* deviceContext )
{
...
int DMSkyPlane::GetIndexCount( )
{
	return m_indexCount;
}

ID3D11ShaderResourceView* DMSkyPlane::GetCloudTexture( )
{
	return m_CloudTexture->GetTexture( );
}

ID3D11ShaderResourceView* DMSkyPlane::GetPerturbTexture( )
{
	return m_PerturbTexture->GetTexture( );
}

float DMSkyPlane::GetScale( )
{
	return m_scale;
}

float DMSkyPlane::GetBrightness( )
{
	return m_brightness;
}

float DMSkyPlane::GetTranslation( )
{
...
bool DMSkyPlane::InitializeSkyPlane( int skyPlaneResolution, float skyPlaneWidth, float skyPlaneTop, float skyPlaneBottom, int textureRepeat )
{
...
void DMSkyPlane::ShutdownSkyPlane( )
{
	// Release the sky plane array.
	if( m_skyPlane )
	{
...
bool DMSkyPlane::InitializeBuffers( ID3D11Device* device, int skyPlaneResolution )
{
	VertexType* vertices;
	...
	result = m_dmd3d->GetDevice()->CreateBuffer( &vertexBufferDesc, &vertexData, &m_vertexBuffer );
	if( FAILED( result ) )
	{
		return false;
	}
...
void DMSkyPlane::ShutdownBuffers( )
{
...
void DMSkyPlane::RenderBuffers( ID3D11DeviceContext* deviceContext )
{
	unsigned int stride;
	unsigned int offset;
	...
	m_dmd3d->GetDeviceContext()->IASetVertexBuffers( 0, 1, &m_vertexBuffer, &stride, &offset );

	// Set the index buffer to active in the input assembler so it can be rendered.
	m_dmd3d->GetDeviceContext()->IASetIndexBuffer( m_indexBuffer, DXGI_FORMAT_R32_UINT, 0 );
...
bool DMSkyPlane::LoadTextures( ID3D11Device* device, WCHAR* textureFilename1, WCHAR* textureFilename2 )
{
	bool result;


	// Create the cloud texture object.
	m_CloudTexture = new DMTexture ;
	if( !m_CloudTexture )
	{
		return false;
	}
	...
	m_PerturbTexture = new DMTexture ;
	if( !m_PerturbTexture )
	{
		return false;
	}
...
void DMSkyPlane::ReleaseTextures( )
{
